...
<div class="crm-block crm-form-block crm-tag-form-block">
   ...
   {/if}
    ...
    <h3>{ts}Coordinator Data{/ts}</h3>                   
    <table class="form-layout-compressed">
      <tr class="crm-tag-form-block-coordinator-id">
        <td class="label">{$form.coordinator_id.label}</td>
        <td>{$form.coordinator_id.html}</td>
      </tr>
      <tr class="crm-tag-form-block-coordinator-start-date">
        <td class="label">{$form.coordinator_start_date.label}</td>
        {if $action eq 4}
          <td>{$form.coordinator_start_date.value|crmDate}</td>
        {else}
          <td>{include file="CRM/common/jcalendar.tpl" elementName=coordinator_start_date}</td>
        {/if}  
      </tr>
      <tr class="crm-tag-form-block-coordinator-end-date">
        <td class="label">{$form.coordinator_end_date.label}</td>
        {if $action eq 4}
          <td>{$form.coordinator_end_date.value|crmDate}</td>
        {else}
          <td>{include file="CRM/common/jcalendar.tpl" elementName=coordinator_end_date}</td>
        {/if}  
      </tr>
    </table>
